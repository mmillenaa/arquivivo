...
// 2. DEFININDO A DIFICULDADE PROGRESSIVA (O Segredo da Emoção)
let intervaloAtual = 150; // Início confortável (Padrão)

if (segundosPassados > 50) {
    intervaloAtual = 30;  // FRENESI FINAL! (Últimos 10 segundos chove muito)
} else if (segundosPassados > 35) {
    intervaloAtual = 70;  // Fase Difícil
} else if (segundosPassados > 20) {
    intervaloAtual = 110; // Aquecimento
}
...
if (this.timer <= 0) {
    // A NOVA MATEMÁTICA: Agora chove até o final da tela de 1280!
    let posX = Math.random() * 1180 + 50; 
    
    // Suas listas de itens
    let itensBons = ['ItemBom', 'ItemBom1'];
    let itensRuins = ['ItemRuim', 'ItemRuim2', 'ItemRuim3'];

    // Sorteia se vai cair algo bom ou ruim
    // DICA: No Frenesi, você pode colocar > 0.3 para cair MAIS itens bons e ajudar o jogador!
    if (Math.random() > 0.5) {
    ...
    // Reinicia o relógio com a nova velocidade dinâmica!
    this.timer = intervaloAtual; 
}

// 4. A CONDIÇÃO DE VITÓRIA PELO TEMPO
if (segundosPassados >= 60) {
    window.jogoAcabou = true;
    
    console.log("Tempo esgotado! O jogador sobreviveu ao Nível 1!");
    
    // Aqui você coloca o comando para ir para o Nível 2 ou Tela de Transição
    // Exemplo: rooms.switch('Nivel2');
}
...
{
this.timer = 150;
this.inicioDaFase = Date.now(); // Marca o início no relógio
}
...
// 1. NAVEGAÇÃO
if (this.direcaoAtual === 0) movX = this.vel;
else if (this.direcaoAtual === 1) movY = -this.vel;
else if (this.direcaoAtual === 2) movX = -this.vel;
else if (this.direcaoAtual === 3) movY = this.vel;

if (!place.free(this, this.x + movX, this.y + movY, 'Obstaculo') || Math.random() < 0.01) {
    let direcoes = [0, 1, 2, 3];
    direcoes.sort(() => Math.random() - 0.5);
    
    for (let d of direcoes) {
        let testeX = 0, testeY = 0;
        if (d === 0) testeX = this.vel;
        else if (d === 1) testeY = -this.vel;
        else if (d === 2) testeX = -this.vel;
        else if (d === 3) testeY = this.vel;
        
        if (place.free(this, this.x + testeX, this.y + testeY, 'Obstaculo')) {
            this.direcaoAtual = d;
            break;
        }
    }
...
this.cronometroPassos += 0.2;
this.angle = Math.sin(this.cronometroPassos) * 4;

// ==========================================
// 4. CONFISCO DE DOCUMENTOS (Polícia - Derrota)
// ==========================================
...
// 2. TEXTURAS DO POLICIAL COM LANTERNA
let novaTex = this.tex;
if (this.direcaoAtual === 0) novaTex = 'Policia-lanterna-direita';
else if (this.direcaoAtual === 1) novaTex = 'Policia-lanterna-cima';
else if (this.direcaoAtual === 2) novaTex = 'Policia-lanterna-esquerda';
else if (this.direcaoAtual === 3) novaTex = 'Policia-lanterna-baixo';
...
// ==========================================
// 4. CONFISCO DE DOCUMENTOS (Polícia Lanterna)
// ==========================================
let livroConfiscado = place.occupied(this, this.x, this.y, 'Coletavel');
...
// ==========================================
// A MÁGICA DA SALA ESVAZIANDO
// ==========================================
// 1. Descobre quem está mais perto de ganhar (Herói ou Polícia)
let maiorPlacar = Math.max(window.livrosHeroi, window.livrosPolicia);

// 2. Calcula qual o limite de livros que podem existir ao mesmo tempo na sala
// Ex: Se o herói tem 49, o limite máximo cai para apenas 1 livro!
let limiteDinamico = 50 - maiorPlacar;

// 3. Conta quantos livros já estão no chão do labirinto neste exato milissegundo
let livrosNoChao = templates.list['Documento'] ? templates.list['Documento'].length : 0;
...
// Define uma margem de segurança para ficar longe das bordas da tela
    let margem = 80; 

    // Tenta achar um lugar livre de paredes invisíveis e bordas
    while (tentativa < 20 && !encontrouLugar) {
        // A MÁGICA AQUI: O sorteio agora foge das bordas (X e Y)
        ...
        // Deixamos a "barriga" dele muito maior para testar as paredes internas (0.8)
        doc.scale.set(0.8); 

        // Ele testa: "Eu, gigante desse jeito, encosto em algum obstáculo?"
        if (place.free(doc, px, py, 'Obstaculo')) {
            encontrouLugar = true; 
            
            // O lugar é super espaçoso! Agora encolhemos ele pro tamanho normal real
            doc.scale.set(0.15); 
            
            // Sorteia a capa do livro
            let roupas = ['Livro-esquerda', 'Livro-baixo', 'Livro-cima', 'Livro-direita'];
            doc.tex = roupas[Math.floor(Math.random() * 4)];
        } else {
            // Se ele bateu na parede, a gente destrói e tenta outro lugar
            doc.kill = true; 
        }
